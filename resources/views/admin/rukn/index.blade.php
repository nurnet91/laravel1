        ...
        $('#GridTable').DataTable({
            ...
            "columns": [
                ...
                },
                {
                    ...
                    name: 'asosiy',
                    render: function(data ,type, row) {
                        var icon = data.asosiy == 1 ? '<i class="fa fa-check fa-lg"></i>' : '<i class="fa fa-close fa-lg"></i>';
                        return '<div style="text-align:center;"><a style="cursor:pointer;" render="{{ $action }}/asosiy/' + data.id + '" onclick="ajaxRequest($(this))">' + icon + '</a></div>';
                    }
                },
                {
                    data: null,
                    name: 'sahifa',
                    render: function(data ,type, row) {
                        var icon = data.sahifa == 1 ? '<i class="fa fa-check fa-lg"></i>' : '<i class="fa fa-close fa-lg"></i>';
                        return '<div style="text-align:center;"><a style="cursor:pointer;" render="{{ $action }}/sahifa/' + data.id + '" onclick="ajaxRequest($(this))">' + icon + '</a></div>';
                    }
                },
